...
class MultiAgentOrchestrator:
    ...
    async def _execute_pipeline(self, workflow_id: str, tasks: List[TaskNode], context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _execute_hierarchical(self, workflow_id: str, tasks: List[TaskNode], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for task, result in zip(tasks, task_results):
            if not isinstance(result, Exception):
                agent_results[task.agent_id] = {
                    "result": result,
                    "execution_time": task.end_time - task.start_time if task.end_time and task.start_time else 0,
    ...
    async def _execute_single_task(self, task: TaskNode) -> Any:
        ...
        try:
            ...
            if (response.get("message_type") == "delegation_response" and 
                ...
                error_msg = response.get("payload", {}).get("error") or response.get("error", "Task execution failed")
    ...
    def _synthesize_collaborative_results(self, gathered_info: Dict[str, Any], processed_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_workflow_summary(self, workflow_id: str, results: Dict[str, Any]) -> str:
        ...
        completed_tasks = sum(1 for task in workflow.values() if task.status == TaskStatus.COMPLETED)
    ...
    def _calculate_workflow_metrics(self, workflow_id: str, execution_time: float) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": sum(1 for task in workflow.values() if task.status == TaskStatus.COMPLETED) / len(workflow) if workflow else 0
        }
