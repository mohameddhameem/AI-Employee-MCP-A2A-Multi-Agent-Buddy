...
class A2AProtocol:
    ...
    def __init__(
        self, agent_id: str, agent_name: str, endpoint: str, secret_key: Optional[str] = None
    ):
    ...
    def _sign_message(self, message: A2AMessage) -> str:
        ...
        signature = hmac.new(
            self.secret_key.encode("utf-8"), message_bytes, hashlib.sha256
        ).hexdigest()
    ...
    def discover_agents(self, broadcast_endpoints: List[str]) -> List[AgentProfile]:
        ...
        for endpoint in broadcast_endpoints:
            try:
                response = requests.post(
                    f"{endpoint}/a2a", json=discovery_message.to_dict(), timeout=5.0
                )
    ...
    def delegate_task(
        self, recipient_id: str, task: str, context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
    ...
    def query_capabilities(self, recipient_id: str) -> Dict[str, Any]:
        ...
        capability_message = self.create_message(
            ...
            recipient_id,
            {"requesting_capabilities": True, "query_timestamp": time.time()},
        )
    ...
    def health_check_agent(self, recipient_id: str) -> Dict[str, Any]:
        ...
        health_message = self.create_message(
            MessageType.HEALTH_CHECK, recipient_id, {"health_check": True, "timestamp": time.time()}
        )
    ...
    def handle_incoming_message(self, message_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": "message_processing_failed", "details": str(e)}
    ...
    def cleanup_expired_requests(self):
        ...
        expired_keys = [
            corr_id
            for corr_id, timestamp in self.pending_requests.items()
